...
from django.urls import reverse # url path설정의 name을 가지고 url을 만들어 주는 함수. 
...
# welcome page
# View 함수 -> 1개 파라미터 선언 (HttpRequest 객체를 받는 파라미터)
...
# template을 저장할 디렉토리를 생성 - app/templates
## polls/templates/polls
def welcome(request):
    now = datetime.now().strftime('%Y년 %m월 %d일 %H시 %M분 %S초')
    # welcome.html 탬플릿을 실행한 결과 html을 받아오기. 
    res = render(
...
## url: polls/list 
## view 함수: list
## template: polls/list.html # polls/templates/[polls/list]
...
## 요청 URL: /polls/vote
## view: vote
## 응답: - 정상 처리: vote_result view를 호출 (redirect 방식)
...
def list(request):
    ...
    - context data (Template에게 전달하는 값)
        - 현재 페이지의 데이터
        - 현재 페이지가 속한 페이지 그룹의 시작페이지 번호, 끝페이지 번호.
        - 현재 페이지 그룹의 시작페이지가 이전페이지를 가지는지 여부 / 이전페이지 번호
        - 현재 페이지 그룹의 끝페이지가 다음페이지를 가지는지 여부 / 다음페이지 번호
    '''
    paginated_by = 10 # 한페이지 당 데이터 개수 
    page_group_count = 10 # 페이지 그룹 당 페이지수 (한번에 몇개 페이지씩 보여줄지)
    # http://127.0.0.1:8000/polls/list?page=3
    current_page = int(request.GET.get('page', 1))

    # Paginator 생성
    question_list = Question.objects.all().order_by('-pk')
    pn = Paginator(question_list, paginated_by)
    ...
    # context_data:dict -> tempalte 호출할때 전달. 
    context_data = {
        'page_range':page_range,
        'question_list':pn.page(current_page)
    }

    # 그룹의 시작페이지의 이전 페이지 유무/ 있다면 페이지번호
