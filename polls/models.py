...
class Question(models.Model):
    ...
    pub_date = models.DateTimeField(auto_now_add=True) # 날짜/시간 타입. True : insert 할때 날짜/시간을 입력 

    def __str__(self):
        return f'{self.id}. {self.question_text}'


# 보기(Choice) 모델 - DBㅡ이 CHOICE 테이블과 연결
class Choice(models.Model):
    choice_text = models.CharField(max_length=200) # 보기문장 
    votes = models.IntegerField(default=0) # 몇명이 선택했는지 저장. 정수형. 
    # Foreign key - QUESTION.id(PK) 참조 
    question = models.ForeignKey(
        Question, # 참조할 model 클래스 
