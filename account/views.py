...
# login - 로그인 사용자의 Model을 session에 저장 (로그아웃할때까지 정보유지)
# logout - session에서 로그인 사용자 Model을 제거 
from django.contrib.auth.decorators import login_required
# login() / lotout(): 로그인 / 로그아웃 처리. - 로그인한 사용자 정보를 session에 추가/제거 
# authenticate(): username(id)/password 를 확인하는 함수.
...
### 요청 방식: GET - 가입 양식화면을 반환(templates/account/create.html)
...
def create(request):
    if request.method == 'GET':
        ...
        # request.FILES: 파일업로드시 업로드된 파일정보.
        ## 객체 저장 -> 요청파라미터들을 attribute로 저장. 검증처리
        
        # 2 요청파라미터 검증
        if form.is_valid(): # 검증 에러가 없으면 True.
            # 3. DB에 저장 (검증 성공)
            user = form.save()  
            print('----------create:', type(user))
            # ModelForm의 save(): Model().save()-insert/update 
            #     반환 - 저장한 정보를 가지는 Model 객체를 반환. 
            ...
            return render(
                ...
                {'form': form}, # form: 요청파라미터를 가지고 검증 결과를 가진 form
            )
...
def user_login(request):
    if request.method == 'GET':
        ...
        # settings.AUTH_USER_MODEL 모델을 기반으로 사용자인증 처리.
        # username과 password가 유효한 사용자 계정이면(select) User모델을 반환. 유효하지 않은 사용지이면 None을 반환. 
        ...
        if user is not None: # 유효한 사용자
            ...
            return render(
                request, 'account/login.html', {'form':AuthenticationForm(), "error_msg":"username이나 password를 다시 확인하세요."}
            )
        
# 로그아웃
## 로그인 안한 상태에서 요청을 받으면 settings.LOGIN_URL로 이동
@login_required
def user_logout(request):
    # login()이 처리한 것들을 모두 무효화
    logout(request)
    return redirect(reverse('home'))
...
def user_detail(request):
    # View에 로그인한 사용자 정보를 조회 -> request.user(모델)
...
def password_change(request):
    ...
    if http_method == 'GET':
        # 비밀번호 변경할 user 정보를 넣어 빈폼을 생성 - 기존 패스워드 확인용.
        ...
        if form.is_valid(): # 검증 통과
            # db update
            user = form.save() # ModelForm.save(): Update/Insert 한 모델객체
